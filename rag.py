...
# --------- config toggles ----------
EXCLUDE_FILENAMES = {"index.md", "glossary.md", "sources.md"}   # skip non-knowledge pages
DEFAULT_TOPN = 20
DEFAULT_K = 6
MMR_LAMBDA = 0.65
...
@dataclass
class Meta:
    doc_id: str     # filename (no path)
    source: str     # full path (for your reference)
    text: str       # chunk text
...
class RAGPipeline:
    ...
    def _mmr(self, query: str, candidates: List[Tuple[int, float]], k: int) -> List[Tuple[int, float]]:
        ...
        doc_embs = self.embed_texts(docs)
        ...
        while len(selected) < min(k, len(candidates)):
            ...
            max_sim = (selected_embs @ doc_embs.T).max(axis=0)
    ...
    def rerank(self, query: str, candidates: List[Tuple[int, float]], k: int) -> List[Tuple[int, float]]:
        ...
        return [(candidates[pos][0], float(sc)) for (pos, sc) in ranked]
    ...
    def generate(self, query: str, selected_ids: List[int]) -> Tuple[str, List[Dict]]:
        ...
        for rank, idx in enumerate(selected_ids, start=1):
            ...
            citations.append(
                {"id": idx, "doc_id": meta.doc_id, "source": meta.source, "text": meta.text}
            )
